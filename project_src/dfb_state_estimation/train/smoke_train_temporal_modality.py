...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Quick smoke wrapper around the unified trainer for temporal modality stage."
    )
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("runs/dfb_state_estimation/train/smoke_temporal_modality"),
    )
    return parser
...
def main() -> None:
    ...
    config = replace(
        ...
        temporal_modality=replace(
            config.temporal_modality,
    ...
    summary = run_training(
        config,
        stage_override="temporal_modality",
