...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Quick smoke wrapper around the unified trainer for single-step stage."
    )
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("runs/dfb_state_estimation/train/smoke_single_step"),
    )
    return parser
...
def main() -> None:
    ...
    config = replace(
        ...
        single_step=replace(config.single_step, step_index=args.step_index),
    )
    summary = run_training(
        config,
        stage_override="single_step",
